...
export default function BookingInfoManagement() {
  ...
  const load = async () => {
    try {
      ...
      toast.error(e?.message || 'Failed to load booking info');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-gray-900">Booking Info Cards</h1>
        <div className="flex gap-2">
          <Button onClick={add} className="rounded-xl"><Plus className="w-4 h-4 mr-2"/>Add Card</Button>
          ...
          {items.map((it, idx) => (
            <motion.div key={it.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
              <Card className="p-4 space-y-3">
                ...
                <div>
                  <label className="text-sm text-gray-600">Icon (lucide name)</label>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" disabled={idx===0 || saving} onClick={()=> reorder(idx, idx-1)}>Up</Button>
                    <Button size="sm" variant="outline" disabled={idx===items.length-1 || saving} onClick={()=> reorder(idx, idx+1)}>Down</Button>
                  </div>
                </div>
              </Card>
            </motion.div>
