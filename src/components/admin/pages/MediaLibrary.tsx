  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="mb-2 text-gray-900">Media Library</h1>
          <p className="text-gray-600">Browse and manage uploaded images & videos</p>
        </div>
      ...
      <Card className="p-4 border-0 shadow-lg flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
        <div className="flex-1">
          <div className="relative">
            ...
            <Input value={search} onChange={(e)=> setSearch(e.target.value)} placeholder="Search by alt, file name, or labels" className="pl-10 rounded-xl" />
          </div>
        </div>
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        {!filtered.length && !loading && <p className="col-span-full text-center text-gray-500 py-10">No media items match your search.</p>}
        {loading && <p className="col-span-full text-center text-gray-500 py-10">Loading media…</p>}
      </div>
      ...
      <Dialog open={uploadOpen} onOpenChange={(o: boolean) => { if(!o){ setUploadOpen(false); resetUpload(); } }}>
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <Input placeholder="Alt text (for images)" value={alt} onChange={(e)=> setAlt(e.target.value)} className="rounded-xl" />
            <Textarea placeholder='Labels JSON array e.g. ["hero","banner"]' value={labels} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>)=> setLabels(e.target.value)} rows={3} />
            {file && (
          ...
          <DialogFooter>
            <Button variant="outline" className="rounded-xl" onClick={() => { setUploadOpen(false); resetUpload(); }}>Cancel</Button>
